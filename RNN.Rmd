...
2. Theory: RNN theory
3. How To: Example implementation
4. Comparing CNN and RNN for time series prediction
5. Sources

# 0. Motivation
From Week 12 lecture, we learnt that Convolutional Neural Networks (CNN) could be used for time series prediction. However, CNN only considers the sequence within the value of **lag** specified by the coder. In this tutorial, we present Recurrent Neural Networks (RNN), which uses all of the past data to predict the future data. This benefit allows us to make prediction better with sequential data.  
