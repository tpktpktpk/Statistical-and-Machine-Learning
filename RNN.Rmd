...
$$
h(t) = f(\textbf{w}^Tx(t)+\textbf{u}^Th(t-1)+b)\\
y(t) = \phi(\boldsymbol{\omega}^Th(t)+\beta)
$$
And for backward calculations, first lets define the loss function:

$$
L(\hat{y},y)=\frac1n \sum (\hat{y}-y)^2 \\
= \frac1n \sum (\phi(\boldsymbol{\omega}^Tf(\textbf{w}^Tx(t)+\textbf{u}^Th(t-1)+b)+\beta)-y)^2
$$
